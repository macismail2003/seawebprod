	...
	validateDataZeroCostEstMulZCEM: function(){
		...
		for(var i=0;i<aZeroCostEstimateZCEM.length; i++){
			...
			if(aZeroCostEstimateZCEM[i].serialNo.trim() == ""){
				...
				//for(var i=0;i<aZeroCostEstimateZCEM.length; i++){
					var serialNo= sap.ui.getCore().byId("idSerNoZCEMInput-col1-row"+i).getValue();
					var dateEntered = sap.ui.getCore().byId("idZcemDateInputZCEM-col2-row"+i).getValue().replace(/-/g,".");
		...
		if(countForNoneZCE == aZeroCostEstimateZCEM.length){
			...
			for(var k=0 ; k < Row.length ; k++){
				urlToCall = urlToCall + " and IEquip" + (k+1) + " eq '" + Row[k] + "'";
			}
		    ...
		    function (data, response){
				...
				for(var i=0;i<aZeroCostEstimateZCEM.length; i++){
					...
					incorrectSerNoDateFlagZCEM = true;
						...
						if(sap.ui.getCore().byId("idSerNoZCEMInput-col1-row"+i).getValue() == "" && sap.ui.getCore().byId("idZcemDateInputZCEM-col2-row"+i).getValue() == ""){
							sap.ui.getCore().byId("idSerNoZCEMInput-col1-row"+i).setValueState(sap.ui.core.ValueState.Error);
							sap.ui.getCore().byId("idSerNoZCEMInput-col1-row"+i).setValue("");
							sap.ui.getCore().byId("idSerNoZCEMInput-col1-row"+i).setPlaceholder("Required");

							sap.ui.getCore().byId("idZcemDateInputZCEM-col2-row"+i).setValueState(sap.ui.core.ValueState.Error);
							sap.ui.getCore().byId("idZcemDateInputZCEM-col2-row"+i).setValue("");
							sap.ui.getCore().byId("idZcemDateInputZCEM-col2-row"+i).setPlaceholder("Required");

							busyDialog.close();
						}
						else if(sap.ui.getCore().byId("idSerNoZCEMInput-col1-row"+i).getValue() == ""){
							sap.ui.getCore().byId("idSerNoZCEMInput-col1-row"+i).setValueState(sap.ui.core.ValueState.Error);
							sap.ui.getCore().byId("idSerNoZCEMInput-col1-row"+i).setValue("");
							sap.ui.getCore().byId("idSerNoZCEMInput-col1-row"+i).setPlaceholder("Required");
						...
						else if(sap.ui.getCore().byId("idZcemDateInputZCEM-col2-row"+i).getValue() == ""){
							sap.ui.getCore().byId("idZcemDateInputZCEM-col2-row"+i).setValueState(sap.ui.core.ValueState.Error);
							sap.ui.getCore().byId("idZcemDateInputZCEM-col2-row"+i).setValue("");
							sap.ui.getCore().byId("idZcemDateInputZCEM-col2-row"+i).setPlaceholder("Required");
							...
							var statusState = "";
							sap.ui.getCore().byId("idSerNoZCEMInput-col1-row"+i).setValueState(sap.ui.core.ValueState.None);
							sap.ui.getCore().byId("idZcemDateInputZCEM-col2-row"+i).setValueState(sap.ui.core.ValueState.None);

							serialNoZCEM = sap.ui.getCore().byId("idSerNoZCEMInput-col1-row"+i).getValue().toUpperCase();
							zcemDateZCEM = sap.ui.getCore().byId("idZcemDateInputZCEM-col2-row"+i).getValue();

							var rxDatePattern = /^(\d{1,2})(\-|-)(\d{1,2})(\-|-)(\d{4})$/;
						    var dtArray = zcemDateZCEM.match(rxDatePattern);


						    if (dtArray == null){
						    	incorrectDateFlagZCEM = false;
						    }
						    else{
						    	incorrectDateFlagZCEM = true;
						    	splitDateZCEM = zcemDateZCEM.split("-");
						    	dateToPassZCEM = splitDateZCEM[2]+splitDateZCEM[1]+splitDateZCEM[0];

						    	selYearZCEM = splitDateZCEM[2];
						    	selMonthZCEM = splitDateZCEM[1];
						    	selDayZCEM = splitDateZCEM[0];
						    }

							if(serialNoZCEM.trim().length > 0 && serialNoZCEM.trim().length != 11)
								incorrectSerNoFlagZCEM = false;
							else
								incorrectSerNoFlagZCEM = true;

							if(selMonthZCEM == 2 && incorrectDateFlagZCEM){
								var isleap = (selYearZCEM % 4 == 0 && (selYearZCEM % 100 != 0 || selYearZCEM % 400 == 0));
								     if (selDayZCEM> 29 || (selDayZCEM ==29 && !isleap)){
								    	 incorrectDateFlagZCEM = false;
								     }
							}
							...
							if(incorrectDateFlagZCEM && incorrectSerNoFlagZCEM){
								var tempZCE = jQuery.grep(aValidateLookupZCE, function(element, index){
						             return element.Equipment.split('-')[0] == sap.ui.getCore().byId("idSerNoZCEMInput-col1-row"+i).getValue().toUpperCase();
						     	});
								if(tempZCE.length > 0){
									if(tempZCE[0].Type == "E"){
